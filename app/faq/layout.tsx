...
async function getFaqMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=faq`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "faq" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "Faq ",
    description: "Learn more Faq mission to help you achieve your goals.",
    keywords: [ "Faq"],
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return await getFaqMeta()
}
