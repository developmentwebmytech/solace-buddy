...
async function getwishlistMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=wishlist`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "wishlist" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "wishlist - Solace Buddy",
    description: "Learn more wishlist Solace Buddy mission to help you achieve your goals.",
    keywords: ["wishlist - Solace Buddy"],
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return await getwishlistMeta()
}
