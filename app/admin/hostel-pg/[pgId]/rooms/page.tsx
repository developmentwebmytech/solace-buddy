...
import { StudentDetailsPopup } from "@/components/admin/student-details-popup"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function RoomsPage() {
  ...
  // Student details popup states
  const [showStudentPopup, setShowStudentPopup] = useState(false)
  const [selectedStudent, setSelectedStudent] = useState(null)
  const [selectedStudentBed, setSelectedStudentBed] = useState(null)
  const [selectedStudentRoom, setSelectedStudentRoom] = useState(null)
  ...
  const fetchRooms = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/admin/vendor-properties/${pgId}/rooms`)
  ...
  const handleEditRoom = async (roomData: any) => {
    try {
      const response = await fetch(`/api/admin/vendor-properties/${pgId}/rooms/${editingRoom._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDeleteRoom = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/vendor-properties/${pgId}/rooms/${deletingRoom._id}`, {
        method: "DELETE",
      })
  ...
  const handleStudentNameClick = (bed: any, room: any, bedIndex: number, e: React.MouseEvent) => {
    e.stopPropagation() // Prevent bed click event from firing

    if (bed.studentName) {
      console.log("[v0] Student name clicked - Bed data:", bed)
      ...
      const studentData = {
        ...
        status: bed.status,
        checkInDate: bed.bookingDate,
        checkOutDate: bed.noticeDate,
      }

      console.log("[v0] Formatted student data for popup:", studentData)

      setSelectedStudent(studentData)
      setSelectedStudentBed(bedIndex + 1)
      setSelectedStudentRoom(room.name)
  ...
  const handleBedUpdate = async (bedData: any) => {
    try {
      ...
      console.log("API URL:", `/api/admin/vendor-properties/${pgId}/rooms/${selectedRoom._id}/beds/${selectedBed._id}`)

      const response = await fetch(
        `/api/admin/vendor-properties/${pgId}/rooms/${selectedRoom._id}/beds/${selectedBed._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center mb-6">
        <Link href="/admin/hostel-pg" className="flex items-center text-blue-600 hover:text-blue-800">
                        ...
                        room.beds.map((bed: any, index: number) => (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              {bed.studentName && (
                                <div
                                  className="text-xs text-blue-600 mt-1 truncate cursor-pointer hover:text-blue-800 hover:underline"
                                  onClick={(e) => handleStudentNameClick(bed, room, index, e)}
      ...
      {selectedStudent && (
        <StudentDetailsPopup
          ...
          onClose={() => {
            ...
            setSelectedStudentBed(null)
            setSelectedStudentRoom(null)
          }}
          student={selectedStudent}
          bedNumber={selectedStudentBed}
          roomName={selectedStudentRoom}
        />
      )}
