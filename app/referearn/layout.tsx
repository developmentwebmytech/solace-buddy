...
async function getreferearnMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=referearn`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "referearn" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "Refer & Earn - Solace Buddy",
    description: "Learn moreRefer & Earn - Solace Buddy mission to help you achieve your goals.",
    keywords: ["Refer & Earn - Solace Buddy"],
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return await getreferearnMeta()
}
