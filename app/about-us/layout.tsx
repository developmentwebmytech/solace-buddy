...
async function getaboutusMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=about-us`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "about-us" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "About Us - Solace Buddy",
    description: "Learn more About Us- Solace Buddy mission to help you achieve your goals.",
    keywords: ["About Us - Solace Buddy"],
  }
}
