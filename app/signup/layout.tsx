...
async function getsignupMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=signup`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "signup" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "signup - Solace Buddy",
    description: "Learn more signup - Solace Buddy mission to help you achieve your goals.",
    keywords: ["signup - Solace Buddy"],
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return await getsignupMeta()
}
