...
async function getfeedbackMeta(): Promise<Metadata> {
  try {
    ...
    const url = `${base}/api/seo-meta?pageType=feedback`
  ...
  try {
    await connectDB()
    const data = await SeoMeta.findOne({ pageType: "feedback" })
    if (data) {
      const keywords: string[] = Array.isArray(data.keywords)
        ? data.keywords
        : typeof data.keywords === "string"
  ...
  return {
    title: "feedback - Solace Buddy",
    description: "Learn more feedback Solace Buddy mission to help you achieve your goals.",
    keywords: ["feedback - Solace Buddy"],
  }
}

export async function generateMetadata(): Promise<Metadata> {
  return await getfeedbackMeta()
}
