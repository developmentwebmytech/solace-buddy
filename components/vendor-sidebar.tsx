...
import { Building2, Home, LogOut, Check, User, Building, BookCheck } from "lucide-react"
...
export function VendorSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  const { vendor, logout } = useVendor() // Added logout from context
            ...
            <div className="flex items-center justify-between p-2">
              <div className="flex items-center gap-2 min-w-0">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sidebar-primary text-sidebar-primary-foreground">
                  <span className="text-sm font-medium">{vendor?.ownerName?.charAt(0) || "V"}</span>
                </div>
                <div className="flex flex-col min-w-0">
                  <p className="text-sm font-medium text-sidebar-foreground truncate">
                    {vendor?.ownerName || "Loading..."}
                  </p>
                  <p className="text-xs text-sidebar-foreground/70 truncate">{vendor?.email || "Loading..."}</p>
                </div>
              </div>
