...
export default function BOOKINGFAQ({ categorySlug = "booking", title }: Props) {
  const [faqs, setFaqs] = useState<FAQItem[]>([])
  const [openItems, setOpenItems] = useState<Record<string, boolean>>({})
  const [loading, setLoading] = useState(true)

  const normalizedSlug = useMemo(() => (categorySlug ?? "booking").toString().trim().toLowerCase(), [categorySlug])
