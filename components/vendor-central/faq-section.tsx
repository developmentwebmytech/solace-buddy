...
export default function PGOWNERSFAQ({ categorySlug = "pgowners", title }: Props) {
  const [faqs, setFaqs] = useState<FAQItem[]>([])
  const [openItems, setOpenItems] = useState<Record<string, boolean>>({})
  const [loading, setLoading] = useState(true)

  const normalizedSlug = useMemo(() => (categorySlug ?? "pgowners").toString().trim().toLowerCase(), [categorySlug])
  ...
  const pageTitle = useMemo(() => {
    ...
    if (normalizedSlug.includes("booking")) return "Booking Related FAQs"
     if (normalizedSlug.includes("pgowners")) return "PG Owners FAQs"
